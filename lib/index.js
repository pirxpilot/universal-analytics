...
var Visitor = module.exports.Visitor = function (tid, cid, options, context, persistentParams) {
  ...
  this.cid = this._determineCid(this.options.strictCidFormat != false, cid, this.options.cid);
  if(this.options.uid) {
...
Visitor.prototype = {
  ...
  _determineCid: function (strict, ...cids) {
    if (strict) {
      for (let cid of cids) {
        let id = utils.ensureValidCid(cid);
  ...
  _checkParameters: function (params) {
    ...
    for (var param in params) {
      if (config.acceptedParameters.indexOf(param) !== -1 || config.acceptedParametersRegex.some(testRe, param)) {
        continue;
      }
  ...
  _translateParams: function (params) {
    if (!params || this.options.skipParamsTranslation) {
      return params;
    }
    let translated = {};
